# src/automation/playwright_controller.py
...
class PlaywrightController:
    ...
    async def navigate_to_url(self, url, max_retries=3, retry_delay=1.0):
        """
        Enhanced navigation method with retry mechanism, history tracking, and fallback navigation.
        ...
        """
        # Use the allowlist from configuration
        if not self.is_url_allowed(url, self.config.allowed_domains):
        ...
        while retries <= max_retries:
            try:
                ...
                if retries > max_retries:
                    logging.error(f"Failed to navigate to {url} after {max_retries} attempts: {e}. Attempting fallback navigation.")
                    fallback_url = "https://www.google.com"
                    try:
                        await page.goto(fallback_url, wait_until="domcontentloaded", timeout=30000)
                        await self._wait_for_page_and_frames_load()
                        self.add_to_navigation_history(fallback_url)
                        logging.info("Fallback navigation to safe URL successful.")
                    except Exception as fallback_error:
                        logging.error(f"Fallback navigation also failed: {fallback_error}")
                    return False
    ...
    async def _wait_for_page_and_frames_load(self, timeout_ms=10000, wait_for_selector: str = None):
        """
        ...
        Args:
            ...
            wait_for_selector: Optional CSS selector that must be present after load
        ...
        try:
            ...
            if wait_for_selector:
                try:
                    await page.wait_for_selector(wait_for_selector, timeout=2000)
                except Exception:
                    logging.warning(f"Selector {wait_for_selector} not found within timeout.")
    ...
    async def get_current_page(self):
        """
        Retrieve the current active page from the browser context.
        (Implement this method according to your application logic.)
        """
        # Example implementation:
        return self.browser_context.pages[0] if self.browser_context and self.browser_context.pages else None
    ...
    def launch_browser_with_profile(self):
        ...
        This method is an instance method and uses the controller's configuration.
        """
